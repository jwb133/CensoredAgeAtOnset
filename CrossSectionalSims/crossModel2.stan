...
model {
  ...
  for (i in 1:n_subj) {
    ...
    real mu_i = alpha0 + b * t_i + c * (ageAtOnset[i]-40);
...
generated quantities {
  ...
  real alpha0Orig, bOrig, cOrig, sdYResOrig;
  bOrig = b*10;
  cOrig = c*10;
  sdYResOrig = sdYRes*10;
  alpha0Orig = alpha0*10+100-40*cOrig;
  
  real c_minus_b;
  c_minus_b = cOrig-bOrig;
  
  real fitted_mean;
  fitted_mean = alpha0Orig -6*bOrig + 6*cOrig;
}
